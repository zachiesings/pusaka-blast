...
/// A field of slowly drifting, twinkling gold motes — cheap ambient "magic dust"
/// for premium screens. Self-animated via a [Ticker]; drop into a Stack with
...
class _SparklePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      // soft dot + a tiny 4-point sparkle for the brightest motes
...
/// A premium sheen that sweeps across [child] (e.g. the gold wordmark) on a
/// loop, using a moving highlight band composited over whatever is rendered.
...
class _ShimmerSweepState extends State<ShimmerSweep> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: _c,
      builder: (context, _) {
        // a highlight band travels left→right; pause off-screen between sweeps
